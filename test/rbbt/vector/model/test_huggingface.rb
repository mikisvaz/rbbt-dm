...
class TestHuggingface < Test::Unit::TestCase
  ...
  def test_embed
    TmpFile.with_file do |dir|
      ...
      model = HuggingfaceModel.new "Embedding", checkpoint, dir, :tokenizer_args => {:max_length => 16}

      assert_equal 768, model.eval("This is dog").length
    end
  end
  ...
  def test_embed_array
    TmpFile.with_file do |dir|
      ...
      assert_equal 768, model.eval_list(["This is dog"]).first.length
    end
  end
  ...
  def test_generate
    TmpFile.with_file do |dir|
      ...
      model = HuggingfaceModel.new "CausalLM", checkpoint, dir, :tokenizer_args => {:max_length => 16}

      iii model.eval("Portugal is")
    end
  end
  ...
  def test_generate_roles
    TmpFile.with_file do |dir|
      ...
      model = HuggingfaceModel.new "CausalLM", checkpoint, dir

      iii model.eval([{role: 'user', content: "Portugal is"}])
    end
  end
